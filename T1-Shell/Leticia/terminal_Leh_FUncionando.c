...
int executa_comando(char c[], int g){
	int fd[2];
	pipe(fd);
	pid_t pid = fork();
	if(pid == 0){
...
int exec_op_independente(char c1[], char c2[]){
	...
	result1 = executa_comando(c1,0);
	result2 = executa_comando(c2,0);
...
int exec_op_ou(char c1[], char c2[]){
	int result;
	result = executa_comando(c1,0);
	//printf("dps ping %d\n", result);
	if(result == 1){ return 1; }
	else if(result == 0){
		//printf("echo\n");
		result = executa_comando(c2,0);
...
int main(){	
	...
	for(i=0; i<qnt_ops; i++){
		...
		if(operador[0]=='|' && operador[1]=='|'){
			...
			if(executa_comando(comandos[i], 0)==1){
				executa_comando(comandos[i+1],0);
				i++;
			}
			else { i++; }
		}else if(operador[0]=='&' && operador[1]=='\0'){
